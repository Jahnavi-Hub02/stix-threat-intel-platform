...
class TAXIIScheduler:
    ...
    def start(self, interval_minutes: int = 30):
        ...
        try:
            ...
            logger.info(
                f"Scheduler started (interval_minutes={interval_minutes}, next_run={next_run})"
            )
    ...
    def trigger_now(self):
        ...
        try:
            ...
            logger.info(f"Manual ingestion triggered: {result}")
            return result
        except Exception as e:
    ...
    def _run_taxii_ingestion(self):
        ...
        try:
            ...
            for server_key, server_config in PUBLIC_SERVERS.items():
                try:
                    logger.info(f"Ingesting from server {server_key}")
                    ...
                    logger.info(
                        f"Server ingestion complete: server={server_key}, stored={result.get('total_stored')}"
                    )

                except Exception as e:
                    logger.error(f"Server ingestion failed for {server_key}: {e}")
                    server_result = {
                        "name": server_key,
                        "status": "failed",
                        "error": str(e),
            ...
            logger.error(f"TAXII ingestion error: {e}")
        ...
        logger.info(f"Ingestion run complete: status={run_info.get('status')}")
        return run_info
    ...
    def _log_ingestion_to_db(
        ...
        try:
            ...
            cursor.execute(
                ...
                (source, status, total_fetched, total_stored, total_duplicates, error_message, completed_at)
                VALUES (?, ?, ?, ?, ?, ?, ?)
                """,
                (
                    "TAXII_SCHEDULER",
