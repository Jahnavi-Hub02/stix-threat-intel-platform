...
def _get_token(client, username="testviewer", role="viewer"):
    """
    Register + login and return the access_token string.

    For admin role: bootstrap the user directly via the DB so the fixture
    is never blocked by any future policy restricting admin self-registration
    through the API.  For viewer/analyst the normal API path is used.
    """
    if role == "admin":
        from app.auth.security import hash_password
        from app.database.db_manager import create_user
        try:
            create_user(username, hash_password("Password123!"), "admin")
        except ValueError:
...
def admin_client_and_token(temp_db):
    """
    Create TestClient + admin user in one sequential flow.

    The admin is inserted directly into the DB (not via API) so the fixture
    is robust against any future policy that restricts admin self-registration.
    Logging in via the API then returns a real signed JWT.
    """
    from fastapi.testclient import TestClient
    from app.api.main import app
    from app.auth.security import hash_password
    ...
    with TestClient(app) as c:
        # Bootstrap admin directly — tables exist after lifespan startup
        try:
