...
def admin_token(api_client):
    """
    Bootstrap an admin user directly via the DB (bypassing the API), then
    log in through the API to obtain a real signed JWT.

    Why bypass the API for creation?
    POST /auth/register requires an existing admin token to create another admin.
    In tests there is no pre-existing admin, so we bootstrap via DB directly.
    api_client already depends on temp_db, so tables exist at this point.
    """
    from app.auth.security import hash_password
    from app.database.db_manager import create_user

    try:
        create_user("admin_fixture", hash_password("Password123!"), "admin")
    except ValueError:
        pass  # already exists in this test's DB — safe to continue
